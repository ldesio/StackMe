...
*!  Stata version 9.0; genyhats version 2, updated Apr'23-May'25 & Jan'26 by Mark from major re-write in June'22
	...
	local optMask = "DEPvarname(name) ITEmname(varname) ADJust(string) EFFects(string) EFOrmat(string) YDPrefix(name) "		/// **
				  + "DPRefix(name) IPRefix(name) LIMitdiag(integer -1) MULtivariate LOGit REPlace NOReplace KEEpmissing"					    	 	//	**
