...
{p 8 16 2}
{opt genyh:ats} {indepvars} [{cmd:,} {it:options}]
{p_end}
...
The second syntax (generally used with unstacked data) creates one yhat variable for each ydprefix, the result 
of regressing the same optioned {opt dep:var} on all of the {indepvars} in the associated {varlist} taken together 
in one multivariate analysis. See {bf:Description}, below, for details.	
...
{phang}
{opt ydprefix} {bf:THERE IS NO ydprefix} option. The {it:f} prefix for yhats generated by 
