...
/*	***************/                          [ MISsing(string) LIMitdiag(integer -1) ROUnd REPlace NODiag PROximities KEEpmissing   ] /// 
											  [ APRefix(string) * ]																	//	***
	...
	if "`contexts'"!="" local contexts = "`contextvars'" 	// Override 'contextvars' characteristic if there was a contextvars option
...
*/											
	...
	forvalues nvl = 1/`nvarlsts'  {							// Cycle thru all varlists
	  ...
	  if "`prfxvars'"!="null" local selfplace ="`prfxvars'" //Any prefix is now in `temp' if `rest' starts with ":"
...
*					       12345678901234567890123456789012345678901234567890123456789012345678901234567890
			  ...
			  if _rc  errexit, msg("Exiting $cmd")			// _rc was non-zero so user is not 'OK'
			    ...
			    while "`name'"==""  {
				  ...
				  else local txt = "UNCHANGED! "			// Else name is unchanged so repeat loop with that word heading the display
				} //next while
...
global errloc "gendi(4)"	
	...
	if "`proximities'"!=""  {								// If proximities were optioned
		...
		foreach var of local non2missing  {					// Cycle thru all d_vars
			...
			if `limitdiag'>`c'  gen x_`var' = `max'-d_`var'	// Noisily invert the distance measures
			else  quietly gen x_`var' = `max'-d_`var'		// Quietly invert the distance measures
		} //next var
...
*					 		 12345678901234567890123456789012345678901234567890123456789012345678901234567890
		...
		else noisily display "Calculating proximities as optd; 'replace' was not optd so distnces will be kept"
	} //endif 'proximities'
