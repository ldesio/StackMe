...
*	global limitdiag = ""									// Empty by default (DO NOT EMPTY BEFORE EXITING wrapper)				**
	...
	if _rc  local stackid = ""								// Holds varname SMstkid or indicates its absence if empty
	else  local stackid = "SMstkid"							// Record name of stackid in local stackid
...
/*	    **************/	  [ prfxtyp(string) * ]		  	// `mask' was establishd in calling `cmd' and preprocssd in codeblk (0.2)
		...
		if ("`nodiag'"!="")  local limitdiag = 0	 			// Retain existing option but with new argument
...
*               		  	  12345678901234567890123456789012345678901234567890123456789012345678901234567890
		...
		if "`if'" != ""  {										// If not empty, calls for `if' when establishng the working dataset
			...
			replace `ifvar' = 1 `if'							//  this indicator whose name is known
...
*		   else  {												// Else this was no previous weight expression and still is none 
	...
	local keepvarstubs = subinstr("`multivarlst'","||","",.)	// trim away pipes (strings in codeblk 3)
																// List of vars that will provide names of vars generated by 'cmd's
																// 'keepvarstubs' will be further pre-processed in codeblk 3
...
pause (2)
	...
	if "`cmd'"=="genstacks" {								// If dataset is about to be stacked ...					
		...
		local frstwrd = word("`multivarlst'",1)				//`multivarlst' was filled with varnames or stubs in (4) above
...
*               		  		  12345678901234567890123456789012345678901234567890123456789012345678901234567890
				...
				if _rc !=0  exit 1							// Exit with error if user says "no"
...
pause (2.1)		
		...
		if "`nostacks'"!="" {
			display as error "'nostacks' cannot be optioned with command genstacks{txt}"
			window stopbox stop "'nostacks' cannot be optioned with command genstacks"
...
*						 	   	   12345678901234567890123456789012345678901234567890123456789012345678901234567890	
			...
			capture confirm variable SMunit
			if _rc==0  {
				display as error "NOTE: Variable SMunit should not already exist in unstacked data{txt}"
				window stopbox stop "Variable SMunit should not already exist in unstacked data"
...
*						      12345678901234567890123456789012345678901234567890123456789012345678901234567890
		...
		if "`dblystkd'"!=""  {								// If data are to be duubly-stacked (SMstkid already exists)
			...
			if _rc == 0  {
				display as error "Variable S2stkid should not already exist in data to be doubly-stacked{txt}"
				window stopbox stop "Variable S2unit should not already exist in data not yet doubly-stacked"
*						 		         12345678901234567890123456789012345678901234567890123456789012345678901234567890				
			}
...
*                		  		 12345678901234567890123456789012345678901234567890123456789012345678901234567890
			   ...
			   local keepvarstubs = subinstr("`keepvarstubs'","`preul'_","",.) // remove prfxs frm kept dta
...
*                		  		      12345678901234567890123456789012345678901234567890123456789012345678901234567890
			   ...
			   local theseprfxvars = "`theseprfxvars' `prfxvars'"
				...
				local keepvarstubs = subinstr("`keepvarstubs'", "`thisInd'", "", .)
			 }													
										
			 capture confirm variable ``opt1''				 	// See if there is a varname in first option
																// Note: need double-quotes to access var that `opt1' points to
			 if _rc==0  local theseprfxvars = "`theseprfxvars' ``opt1''" 
																// If so, save it in 'theseprfxvars' as though it was a prefix
		 } //end else
...
*                		  		12345678901234567890123456789012345678901234567890123456789012345678901234567890
		...
		if strpos("`postcolon'","`prfxvars'")>0  & "`prfxtyp'"=="var"  { 
			 display as error "Prefix var appears in list of indepvars{txt}"
		...
		if "`prfxvars'"!="" & "`cmd'"!="genyhats" & "`cmd'"!="gendummies"  {
			...
			if "``opt1''"!=""  {								// Note double '' to establish whether `opt1' points to anything
				if `limitdiag' display as error "NOTE: Indicator prefix 'prfxvars:' supercedes indicator option ``opt1'' {txt}"
*                		  		  				 1234567890123456789012345678901234567890123456789012345678901234567890123456789
    ...
    local keepvarstubs = stritrim(subinstr("`keepvarstubs'",":"," ",.))	// Need spaces to replace colons; then trim extra spaces
	
	local keepvars = strtrim(stritrim( "`keepoptvars' `keepifwtvars' `keepimpliedvars' `keepstackvars' `keepprfxvars' `keepvarstubs' `origunit'"))
...
noi display "multivarlst: `multivarlst'"
noi display "prefixvars: `prefixvars'"
noi display "keep: `keep'"
...
*	*****************										// (May include SMitem or S2item generated just above)
	...
	if _rc!=0 {
		display as error "Program error"
		exit 1
	}
	...
	if "`cmd'"=="gendist" {									// Commands should not create a prefixed-var that already exists
		...
		foreach var of local keepvarstubs  {				// Only vars to be used as stubs need be checked
			capture confirm variable d_`var'
			if _rc==0  {
				if "`dprefix'"==""  {						// If relevant prefix is not to be changed ...
					display as error "Variable `pfx'_`var' already exists; use option `pfx'prefix to change the prefix{txt}"
*                		  		      12345678901234567890123456789012345678901234567890123456789012345678901234567890
					window stopbox stop ///
					    "Need to option a different prefix for displayed variable since the prefixed var already exists"
				} //endif `dprefix'
			} //endif _rc
		} //next var
	...
	if "`cmd'"=="geniimpute"  {								// See comments above for explication
		...
		foreach var of local keepvarstubs  {
			capture confirm variable yi_`var'
			if "`iprefix'"==""  {
				if _rc==0  {
					display as error "Variable yi_`var' already exists; use option iprefix to change the prefix{txt}"
					window stopbox stop ///
					"Need to option a different prefix for displayed variable since the prefixed var already exists"
				}		
			} //endif `dprefix'
	...
	if "`cmd'"=="genyhats"  {
		...
		foreach var of local keepvarstubs  {
			  if `isprfx'==0  {								// If this is NOt a multivariate procedure (`isprfx'==0)
				  capture confirm variable yi_`var'
				  if _rc==0  {
					  display as error "Variable yi_`var' already exists; use option yiprefix to change the prefix{txt}"
*                		  		      12345678901234567890123456789012345678901234567890123456789012345678901234567890
					  window stopbox stop ///
						"Need to option a different prefix for displayed variable since the prefixed var already exists"
				  } //endif
			   ...
			   else  {										// Else this varlist IS prefixed with yd_ prfx
				  capture confirm variable yd_`var'
				  if _rc==0  {
					 display as error "Variable yd_`var' already exists; use option ydprefix to change the prefix{txt}"
					 window stopbox stop ///
						"Need to option a different prefix for displayed variable since the prefixed var already exists"
				  }  
			  } //endelse isprfx
	...
	local keepvars = strtrim(stritrim( ///
		"`keepoptvars' `keepifwtvars' `keepimpliedvars' `keepstackvars' `keepprfxvars' `keepvarstubs' `origunit'"))
...
pause (6)	
	...
	local limit = `limitdiag'			
	if `limitdiag'<0 local limit = .						 	  	  // Overwrite `limitdiag' with big number if =-1
	...
	forvalues c = 1/`nc'  {									 	  	  // Cycle thru successive contexts (`c')
	    ...
	    preserve								 					  // Preserve the working data
		   ...
		   if "`extradiag'"!="" {								      // If extra diagnostics were optioned ...
			   
			   if `limit'>=`c'  {									  // If `varmis'!="" note will be displayed after `cmd'P call																						 

				   if "`cmd'"=="genstacks" local vartest = "`impliedvars'" 
	       ...
	       forvalues nvl = 1/`nvarlst'  {							  // HERE ENSURE WT EXPRESSIONS GIVE VALID RESULTS
		     ...
		     if "`wtexp"!=""  {										  // Now check if weight var is not missing in this context
			   ...
			   if _rc  {											  // An error can only be due to the 'weight' expression
				   ...
				   if _rc==2000 display as error "Stata reports 'no obs' error; perhaps weight var is missing in context `lbl' ?"
									            //12345678901234567890123456789012345678901234567890123456789012345678901234567890 
				   else  {
					  local l = _rc
					  display as error "Stata reports error `l' in context `lbl'"
				   }
error `l'
...
pause (7)
										...
										//	   NOTE that 'forvalues `c'=' remains in effect
set tracedepth 4																		
		   ...
		   if ! $exit  {												// So long as an error was not flagged just above,..
				...
				quietly keep `tempexp' `c'==`ctxvar' `inexp' 	
...
*				 **************											// 
		   ...
		   else  {											 			// Else error exit from `cmd'P, restore origdata then exit
				  capture restore										// Not sure what can go wrong but evidently something did
				  quietly use `origdta', clear
				  error 1
		   }
				     
		   if `limit'>=`c' & "`varmis'"!="" {							// `varmis' got its value before call on `cmd'P
			   ...
			   if "`extradiag'"!=""  {
					  ...
					  if `nwrds'==2 local errtxt = "`wordl'"
				   	  if `nc'>1  display as error "NOTE: No observations for var(s) `word1' `errtxt' in context `lbl'{txt}"
					  if `nc'==1 display as error "NOTE: No observations for var(s) `word1' `errtxt'{txt}"
		...
		restore															// Restore full dataset from which to extract next context
																		...
																		// REMAINNG LINES SHOULD BE REDUNDNT IF THIS WAS lastvarlist ***
...
*	if `more' {															// REDUNDANT OPENING AND CLOSING LINES						 ***
	  ...
	  if "`multiCntxt'"!= ""  {											// If there ARE multi-contexts (local is not empty) ...
																		// Collect up & append files saved for each contxt in codeblk (6)
																		// (If there was only one context then just one was saved)
		 preserve

			quietly use `temp', clear									// Tempfile in which the first context was saved in codeblock (6)
			...
			forvalues i = 1/`napp'  {
				...
				erase `a'												// (_mkcross ids are adjacent)	
			} //next 'i'
...
pause (9)
	...
	local var1 = word("$multivarlst",1)							// ENSURE $multivarlst ALWAYS STARTS W RELEVANT VARS				***
	...
	if "`cmd'"=="genyhats"   {
	    ...
	    if _rc==0  capture rename (yi_*) (YI_*)					// (after any processing may be needed for newly prefixed vars)
...
capture program drop isnewvar								// APPARENTLY NO LONGER CALLED FROM gendist VERSION 2
