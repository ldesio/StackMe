...
*/															// (above comments regard origins of gendu varlst componnts parsd in wrappr)
		...
		foreach var of local vars  {				  		// Cycle thru varlist(s) in `anything' (wrapper ensures 1 stub per var)
		  ...
		  if _rc  local notvar = "`notvar' `var'"			// If variable is NOT numeric, accumulate in local 'notvar'
		  	 ...
		  	 local varstr = `var'[_N]
			 capture confirm integer number "`var'"
			 if _rc  local notint = "`notint `var'"
		  }
		   
		  else  {											// Else variable IS numeric..
...
global errloc "genduP(5)"									// Global that keeps track of execution location for benefit of 'errexit'
		    ...
		    foreach val of local values  {						// Cycle thru all levels of `var'
			  ...
			  if ("`includemissing'"=="") & `val'>=.  continue	// Continu w nxt val if this one is missng (note 'includem' opt, above)
			  capture confirm number `val'						// (this is done if `includemissing' was NOT optioned)
			  if _rc  & "`val'"!="." {							// If `val' is not numeric and not missing..
				 local notnum = "`notnum' `val'"				// Record it in list of non-numeric suffixes (DK WHY WE DO THIS)
				 continue										// Continue with next value
			  }
*			  **************************
			  if "`val'"!="." gen du_`out'`val' =(`var'==`val') // This is the money cmd that creates a new var for each value			***
*			  **************************						// NOTE that, while outcome is stubname `out', input is still 'var' 
			  else  continue									// Else, rather than create var with missing value, we omit this var
...
global errloc "genduP(5.1)"										// Global keeps track of execution location for benefit of 'errexit'
			  ...
			  if ("`includemissing'"!="") {						// See if user optioned assigning values of 0 for missing data
			  
				 qui replace du_`out'`val' = 0 if du_`out'`val'>=.
...
*                		     12345678901234567890123456789012345678901234567890123456789012345678901234567890
		  ...
		  } //endelse (is a variable)
		   
	    } //next `var'
		
	    if "`notvar'"!=""  {
		  local txt = "Gendummies found non-numeric variables: `notvar'; drop and continue?"
		  dispLine "{err}`txt'{txt'}"
		  capture window stopbox rusure "Gendummies found non-numeric variables: `notvar'; drop and continue?"
		  errexit, msg("`txt'")								 	  // Option-format errexit only sends 'txt' to stopbox
		  if _rc  {
			 errexit "Lacking permission to drop non-integer variables"
		  }
...
capture }														  // CLUGES PREVENT "not a valid command name" on 'errexit'
																  // (or "unexpected end of file" – 'errexit can mess w nesting structure)
