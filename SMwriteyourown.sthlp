...
equation it implements so as to produce a program that calculates some other systemic measure. In 
...
stackMe's {help stackme##Genericvariable:Affinity measures} help text for additional details.{p_end}
...
them next to each other. The two "iss" (for issue) variables are not mentioned in this {help {ul:genst}acks} 
command but they are duplicated onto each row of the long-format dataset just as are all other variables 
in the dataset that are not mentioned in the {cmdab:genst:acks} command, becoming what we will refer to as 
{bf:"subordinate batteries"} (a step on the way to becoming "doubly-stacked batteries"). A second 
{cmdab:genst:acks} command would complete the creation of a doubly-stacked dataset by operating on 
...
{p 2 2 2}
The "i" and "j" indices, shown in the illustration, are not used by {cmdab:genst:acks} but relate 
...
observations(units) other than by their sequential position, {cmd:stackMe}'s {cmdab:genst:acks} 
provides a supplementary means of identifying specific stacks by means of another {cmd:stackme} 
...
offer benefits in terms of data storage and execution time when conducting exploratory studies, or in 
order to use subordinate battery items as as control variables. This is the substantive topic explored 
in the next section of this illustrative helpfile, the next step on the way to contributing user-supplied 
programs that will run under {help stackMe}.{p_end}


{space 2}{title:Baby steps (1): simple strategies for summarizing subordinate battery values}
...
the only remaining option is the string of characters "po_" that the {cmd:SMpolarizIndex} command (or any 
other user-written command) would be programmed to recognize as the prefix to be used for the variable names 
that will identify the new measures, one measure for each of the parties whose left-right locations were held 
by the variables in the {cmdab genpl:ace} variable list, a feature that is common to all stackMe commands. 
This call is made without need for {cmdab:genpl:ace} to know what will be done with information transferred 
to the optioned program. A few paragraphs below we will see the Stata code executed by {cmdab:genpl:ace} in 
response to the optioned {opt call}; but first we will need a gentle introduction to Stata programming. 
Before we take that step we should mention a strategy for analysing doubly-stacked data that does not 
involve any programming skills beyound those needed to reach this point with {bf:stackMe} commands.


{space 2}{title:Baby steps (2): simple strategies for analysing "dual-affinities" in doubly-stacked data}

{p 2 2 2}
Doubly-stacked data, as already explained, involves stacking an already stacked dataset, so that a second 
stacked dimension is nested within the first (we used the example of issues nested within parties and its 
equivalent: parties nested within issues). As we pointed out, such a doubly-stacked dataset permits research 
into the links between voters and political parties by way of the political issues that concern both voters 
and parties. Analysing those dual linkages can be done with no new skills simply by generating a dummy variable 
that indicates (codes =1) the party promoting that issue by any voter supporting the same issue. It should 
be evident that proximities can be used in the same way, simply by multiplying the extent to which a party 
empphasises an issue by the proximity that each supporter feels towards the same issue (and similarly for 
yhats). But the possibility of investigating dual-affinities opens up the prospect of quite new types of 
affinity whose measurement might call for additional skills.


{space 2}{title:Stata programming for beginners (this section can be skipped by those who do not need it)}
{p 2 2 2} 

/* Here we insert a flow diagram to illustrate how subroutine calls fit in the general picture */

{p 2 2 2}Users unfamiliar with Stata programming should not hesitate to 
...
{col 5}{bf:gettoken cmd args : call} {col 43} // Split {opt call} option-string into 1st word (cmd) & rest (opts)
...
option that ends the {Cmdab:genpl:ace} command that was illustrated towards the end of the previous section 
...
{p 2 2 2}
Next comes some of the Stata code contained in the user-written program in response to the SMpolarizindex call:{p_end}
...
{space 4}{bf:args opts} {col 42} // Split the arguments (produced above) used when invoking this `cmd'
{space 4}	   			{col 42} // Next line refers to (global) varlist (counterpart to local)
{space 4}{bf:gettoken vars wt:(global)varlist, p("[")} {col 45}// Split varlist from appended weight expression (the 
{space 4}	   			{col 45} //  global varlist was supplied by {cmd:genplace}); the weight 
{space 4}	   			{col 45} //  expression will be needed by some users; the ', p("[")'
{space 4}	   			{col 45} //  suffix provides a parsing character that replaces the
{space 4}	   			{col 45} //  default space used earlier to define word boundaries
{space 4}	   			{col 45} 
{space 4}{bf:local prfx = word("`opts'",1)} {col 45} // Get what we need from the generic `opts' string
{space 4}	   			{col 45} // (for this `cmd', only a prefix string was included)

{space 4}{bf:foreach var of varlist vars} {  {col 45} // Cycle thru each `lrp' in the {bf:genplace} varlist
{space 4}    :			{col 45} // (local `vars' holds left-right placements of each party)
{space 4}    :
