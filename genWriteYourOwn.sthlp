...
two variables ({it:stackme_unit}, equivalent to the {bf:{it:i}} index, and {it:stackme_item}, 
...
{space 3}{bf:Note} that the Stata {help reshape}'s {bf:{it:j}} index does not identify reshaped observations
(units) other than by their sequential position, whereas {cmd:genstacks}' {bf:{it:stackme_item}} identifies 
...
{p 2 2 2}
would generate, separately for each context and stack, a p_-prefixed (the default if no other prefix 
is optioned) variable that recorded the largest value 
...
{p 4 4}{cmd:genplace lrp1-lrp9,} {opt context(cntry year)} {opt stackid(stkid)} {bf:{opt call(polarindex po_)}}{break}
...
{col 5}{bf:gettoken cmd opts : call}{col 47}// Split the call option-string into command & options
{col 5}{bf:local vars = "`vars' [`weight']"}{col 47}// Extend current varlist with any weight expression
{col 5}{bf:`cmd' `vars',} {bf:opts("`opts'")}{col 47}// Issue `cmd' with extended varlist & specified options
...
{space 4}	   			{space 14}//  the current varlist and weight expression to the call)
...
{space 4}{bf:gettoken vars weight:anything, parse("[")} // Split `anything' into vars and weight (unused here)
...
{space 4}    '
{space 4}    {bf:generate `prfx'`lrp' = ...} {space 11}// Assign each result to the same (now "po_"-prefixed) var
...
any number of same-length lists (provided the length concerned was held in a previous option). So the 
