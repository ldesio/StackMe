...
{synopt :{opt apr:efix(name)}}prefix that overrides all default or user-optioned prefixes for variables generated 
...
of being found empirically (cf. King et al. 2001). If this is not considered adequate, it is possible to 
iimpute multiple different datasets, each one separately saved to be later imported into Stata's {help mi}. 
suite of commands. Alternatively, Stata's {help mi} suite of commands can be employed with data that have 
been pre-processed by {cmd:StackMe}; but this would result in an (arguably unnecessary) m-fold increase in 
the number of observations for later analysis in what might often already be a very large dataset.
...
{phang}
{opt keepmissing} if specified, prefix that replaces all existing name prefixes for variables generated by 
this command (the "a" in "apprefix" stands for "all").{p_end}
